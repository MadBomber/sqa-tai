# CDL_CDL_MARUBOZU (Marubozu)

## Overview

The Marubozu is a continuation candlestick pattern used in technical analysis to identify potential trading opportunities. The pattern consists of Long candle with no shadows (or very small shadows). This pattern provides traders with insights into market psychology and potential price movements when properly confirmed.
...
The Marubozu pattern measures:
- **Market Psychology**: Strong directional conviction from open to close
...
### Pattern Family
The Marubozu belongs to the candlestick pattern family used for:
...
Use Marubozu across multiple timeframes:
